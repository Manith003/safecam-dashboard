...
//         </div>
//         <Badge variant="outline" className="border-[#00C853] bg-[#00C853]/10 text-[#00C853]">
...
//         <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.45 }} className="w-full max-w-2xl">
//           <Card className="overflow-hidden border-gray-800/50 bg-gradient-to-br from-[#161B22] to-[#0F1218]">
//             <div className="relative aspect-video bg-gray-900">
//               <div className="flex h-full items-center justify-center">
//                 <Radio className="h-16 w-16 animate-pulse text-[#007BFF]" />
//               </div>
...
//                 <Badge variant="outline" className="border-[#007BFF] bg-[#007BFF]/10 text-[#007BFF]">Active</Badge>
//               </div>
...
export function LiveCameras() {
  ...
  useEffect(() => {
    socket.on("video_frame_broadcast", (data) => {
      if (data.deviceId === camera.id) {
        setFrame(data.frame);
      }
    });

    return () => {
      socket.off("video_frame_broadcast");
    }
  }, []);
  return (
    <div className="h-full overflow-y-auto bg-neutral-800 p-6">
